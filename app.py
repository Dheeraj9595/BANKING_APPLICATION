    ...
    if session['usert'] == "executive":
        if request.method == "POST":
            ...
            if result is None:
                ...
                if result == 0:
                    query = Customers(cust_id=110110000, cust_ssn_id=cust_ssn_id, name=name, address=address, age=age,
                                      state=state, city=city, status='activate')
                else:
        ...
        if request.method == "POST":
            ...
            if result is not None:
                ...
                if result is None:
                    ...
                    if result == 0:
                        query = Accounts(acc_id=360110000, acc_type=acc_type, balance=amount, cust_id=cust_id,
                                         status='active', message=message, last_update=datetime.datetime.now())
                    else:
                        query = Accounts(acc_type=acc_type, balance=amount, cust_id=cust_id, status='active',
                                         message=message, last_update=datetime.datetime.now())
                    db.add(query)
                    db.commit()
                ...
                if src_type != trg_type:
                    ...
                    if src_data is not None and trg_data is not None:
                        if src_data.balance > amount:
                            ...
                            db.commit()
                            temp = Transactions(acc_id=src_data.acc_id,
                                                trans_message="Amount Transfered to " + str(trg_data.acc_id),
                                                amount=amount)
                            db.add(temp)
                            db.commit()
                            sql_query = text("update accounts set balance = :b where cust_id = :a and acc_type = :t")
                            ...
                            db.commit()
                            temp = Transactions(acc_id=trg_data.acc_id,
                                                trans_message="Amount received from " + str(src_data.acc_id),
                                                amount=amount)
                            db.add(temp)
                            db.commit()

                            flash(f"Amount transfered to {trg_data.acc_id} from {src_data.acc_id} successfully",
                                  'success')
                        else:
        ...
        if acc_id is not None:
            ...
            if data:
                if ftype is None:  # Check for provide pdf file as default
                    ...
                    response = Response(output, mimetype="application/ms-excel",
                                        headers={"Content-Disposition": "attachment;filename=statment.xls"})
                    return response
            else:
...
def customerlog():
    ...
    if session['usert'] == "executive":
        if request.method == "POST":
            ...
            sql_query = text(
                "select log_message,time_stamp from customerlog where cust_id= :c ORDER by time_stamp desc")
            data = db.execute(sql_query, {'c': cust_id}).fetchone()
            t = {
            ...
            dict_data = []
            sql_query = text(
                "SELECT customers.cust_id as id, customers.cust_ssn_id as ssn_id, customerlog.log_message as message, customerlog.time_stamp as date from customerlog JOIN customers ON customers.cust_id = customerlog.cust_id order by customerlog.time_stamp desc limit 50")
            data = db.execute(sql_query).fetchall()
            for row in data:
                t = {
                    "id": row.id,
